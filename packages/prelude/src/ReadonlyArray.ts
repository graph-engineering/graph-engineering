...
export const fromHead = <A>(head: A): ReadonlyArray<A> => [head];

export const nonNullables = <A>(
  as: ReadonlyArray<A>
): ReadonlyArray<NonNullable<A>> =>
  as.reduce<ReadonlyArray<NonNullable<A>>>(
    (as, a) => Option.fromNullable(a).fold(as, (a: any) => [...as, a]),
    []
  );

export const nullableCons = <A>(
  head?: Maybe<A>,
  tail?: Maybe<ReadonlyArray<A>>
): ReadonlyArray<A> =>
  These.fromNullables(head, tail)
    .getOrElse(These.that([]))
    .fold(fromHead, Fn.identity, cons);

/*
  Functions are redeclared with `ReadonlyArray` instead of `Array` below; it
  would be nice if fp-ts natively supported `ReadonlyArray`...
*/
