...
import { RatioTableWithNumbersOrRelationshipFunctions, RatioTableWithOnlyNumbers } from './types'

// TODO: make more typesafe
// TODO: curry when available
export const compareObjects = <T extends object, K extends object>(include: boolean, input: T, lookup: K): T =>
...
const objectIsNotEmpty = <T>(obj: T): Either.ErrorOr<T> =>
	Either.fromPredicate<Error, T>(
		...
		Error.fromL('Flexible Calculator must have at least one key that exists in the provided ratio table.')
	)(obj)
