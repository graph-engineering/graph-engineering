...
*/
export const toDate = (english: string): Result.Result<Moment.Moment> =>
	'right now immediately instantly at once'.includes(english.toLowerCase())
        ...
        Apply.liftA3(Either.either)<number, Unit, Tense, Moment.Moment>(
          amount => unit => tense => 
             tense === Tense.PAST
...
*/
export const toDuration = (english: string): Result.Result<Moment.Duration> =>
	toDate(`${english} ago`).map(date =>
		Utils.duration(Utils.ongoingInterval(date))
	)
